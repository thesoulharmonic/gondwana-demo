...
function EditProductPage() {
    ...
    function handleSubmit(e) {
        e.preventDefault();
        if (!name || !description || !price || !category || !artist || !images.length) {
            return alert("Please fill out all the fields");
        }
        updateProduct({ id, name, description, price, category, artist, images }).then(({ data }) => {
            if (data.length > 0) {
                setTimeout(() => {
                    navigate("/");
                }, 1500);
    ...
    function showWidget() {
        const widget = window.cloudinary.createUploadWidget(
            {
                cloudName: "dkjqcas6i", //Cloudinary API name
                uploadPreset: "gondwana", //upload reset set in CLoudinary setting
            },
            (error, result) => {
                ...
                <Col md={6} className="new-product__form--container">
                    <Form style={{ width: "100%" }} onSubmit={handleSubmit}>
                        ...
                        <Form.Group className="mb-3">
                            ...
                            <Form.Control type="text" placeholder="Artist" value={artist} required onChange={(e) => setArtist(e.target.value)} />
                        </Form.Group>
                        ...
                        <Form.Group className="mb-3" onChange={(e) => setCategory(e.target.value)}>
                            ...
                            <Form.Select value={category}>
                                ...
                                <option value="tickets">tickets</option>                            </Form.Select>
                        </Form.Group>
